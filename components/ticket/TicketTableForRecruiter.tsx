...
import {useTicketsByCandidate, useTicketsByRecruiter} from "@/lib/hooks/useTickets";
...
export default function TicketTableForRecruiter() {
  ...
  const ticketRecruiterQuery=useTicketsByRecruiter(user?.roleDetails.recruiterId);
  console.log("ticketData: ",ticketRecruiterQuery.data)

  const users=ticketRecruiterQuery.data || [];
  ...
  if(ticketRecruiterQuery.isPending) return <LoadingComponent />
  if(ticketRecruiterQuery.isError) return <ErrorComponent />
