...
import {useUpdateJobMutation} from "@/lib/hooks/useJobs";
...
const VacancyTable: ({filter, vacancies}: { filter: any; vacancies: Job[] }) => React.JSX.Element = ({
                                                                                                         filter,
                                                                                                         vacancies
                                                                                                     }) => {
        ...
        (vacancy: Vacancy, columnKey: string) => {
            ...
            switch (columnKey) {
              ...
              <span className="text-lg text-default-400 cursor-pointer active:opacity-50"
                    onClick={() => (location.href = `/jobs/${vacancy.jobId}`)}>
                <EyeIcon/>
              </span>
                            </Tooltip>
                            {vacancy.status == "LIVE" && <>  <Tooltip content="Edit Vacancy">
              <span className="text-lg text-default-400 cursor-pointer active:opacity-50"
              ...
              <span className="text-lg text-danger cursor-pointer active:opacity-50"
                    onClick={() => handleUnpublishing(vacancy.jobId!)}>
                <DeleteIcon/>
              </span>
                                </Tooltip>
                            </>
...
//handle unpublishing
    const handleUnpublishing = (jobId: string) => {
        Swal.fire({
            title: "Do you want to unpublish the job?",
            icon: "warning",
            customClass: {
        ...
        >
            <TableHeader columns={headerColumns}>
                {(column) => (
                    <TableColumn
                        key={column.uid}
                        align={(column.uid === "title" || column.uid === "id") ? "start" : "center"}
